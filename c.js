...
#define MODULOUID 199999
#define MODULOPID 241
...
int mappingUserID(const char *str) {
    ...
    for (int i = 0; str[i] != '\0'; i++) {
        result = (result * 31 + (str[i] - '0')) % MODULOUID;
    }
...
int mappingpID(const char *str) {
    ...
    for (int i = 0; str[i] != '\0'; i++) {
        result = (result * 37 + (str[i] - '0')) % MODULOPID;
    }
...
int main()
{
    ...
    while(1)
    {
        ...
        if (!strcmp(mtea, "?number_submission_period"))
        {
            ...
            printf("%d\n", a[timeToSeconds(endPoint)] - a[maxx(0, timeToSeconds(startPoint) - 1)]);
        }
    }
    return 0;
}
